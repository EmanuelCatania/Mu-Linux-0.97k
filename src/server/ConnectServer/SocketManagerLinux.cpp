...
bool CSocketManager::DataRecv(int index, IO_MAIN_BUFFER* lpIoBuffer)
{
	...
	while (true)
	{
		...
		else
		{
			...
			LogAdd(LOG_RED, "[SocketManager] Protocol header error (IP: %s, Header: %x)", lpClientManager->m_IpAddr, lpMsg[count]);
			return false;
		}
		...
		{
			CClientManager* lpClientManager = &gClientManager[index];
			LogAdd(LOG_RED, "[SocketManager] Protocol size error (IP: %s, Header: %x, Size: %d, Head: %x)", lpClientManager->m_IpAddr, header, size, head);
			return false;
		}
...
static bool FlushSendBuffer(int epollFd, int index, CClientManager* lpClientManager, IO_SEND_CONTEXT* lpIoContext)
{
	while (lpIoContext->IoSize > 0)
	{
		while (lpIoContext->IoMainBuffer.size < lpIoContext->IoSize)
		{
			ssize_t sent = send(lpClientManager->m_socket,
			...
			{
				UpdateEpollWrite(epollFd, lpClientManager->m_socket, index, true);
				return false;
			}
		...
		if (lpIoContext->IoSideBuffer.size <= 0)
		{
			...
			UpdateEpollWrite(epollFd, lpClientManager->m_socket, index, false);
			return true;
		}
...
bool CSocketManager::DataSend(int index, BYTE* lpMsg, int size)
{
	...
	FlushSendBuffer(this->m_epollFd, index, lpClientManager, lpIoContext);
...
void CSocketManager::Disconnect(int index)
{
	...
	epoll_ctl(this->m_epollFd, EPOLL_CTL_DEL, lpClientManager->m_socket, nullptr);
	...
	lpClientManager->m_socket = INVALID_SOCKET;
...
void CSocketManager::OnRecv(int index, DWORD, IO_RECV_CONTEXT* lpIoContext)
{
	...
	while (true)
	{
		...
		ssize_t received = recv(lpClientManager->m_socket, &lpIoContext->IoMainBuffer.buff[lpIoContext->IoMainBuffer.size], capacity, 0);
...
DWORD CSocketManager::ServerWorkerThread(CSocketManager* lpSocketManager)
{
	...
	while (lpSocketManager->m_running)
	{
		...
		for (int i = 0; i < count; ++i)
		{
			...
			{
				lpSocketManager->OnRecv(index, 0, lpClientManager->m_IoRecvContext);
			}

			if (events[i].events & EPOLLOUT)
			{
				lpSocketManager->OnSend(index, 0, lpClientManager->m_IoSendContext);
			}
		}
	}
