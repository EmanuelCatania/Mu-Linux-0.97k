...
- Search strategy: source constant `LoginInputPasswordCall`, exact executable
...
- Data references: surrounding code accesses the native input state after the
  call.
...
- Stack cleanup: the surrounding native flow adjusts the stack.
...
- Original values or bytes: `E8 0B D9 F5 FF`.
...
- Disassembly evidence: exact `CALL rel32` bytes resolve to `0x0047F0B0` and
  execution resumes at `0x005217A5`.
