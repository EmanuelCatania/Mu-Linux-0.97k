...
void CItemBagEx::Load(const char* path)
{
	...
	for (std::map<int, ITEM_BAG_EX_INFO>::iterator it = this->m_ItemBagInfo.begin(); it != this->m_ItemBagInfo.end(); it++)
	{
		if ((GetLargeRand() % 10000) < it->second.DropRate)
		{
			...
			for (std::vector<ITEM_BAG_EX_DROP_INFO>::iterator DropInfo = it->second.DropInfo.begin(); DropInfo != it->second.DropInfo.end(); DropInfo++)
			{
				...
				{
					RandomManager.AddElement(reinterpret_cast<intptr_t>(&(*DropInfo)), DropInfo->SectionRate);
				}
			}

			intptr_t dropPtr = 0;

			if (RandomManager.GetRandomElement(&dropPtr) == 0)
			{
				return 0;
			}

			ITEM_BAG_EX_DROP_INFO* lpItemBagDropInfo = reinterpret_cast<ITEM_BAG_EX_DROP_INFO*>(dropPtr);
